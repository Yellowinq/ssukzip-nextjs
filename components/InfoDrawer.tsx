...
const InfoDrawer: React.FC<Props> = ({marker, isOpen, onClose}) => {
  ...
  const { data, error } = useSWR<Array<Reply>>(
    () => `https://ssukzip.deta.dev/review/info/${marker.content.id}`,
    fetcher,
    { refreshInterval: 500 }
  )
  ...
  const handleSubmit: React.MouseEventHandler<HTMLButtonElement> = async (e) => {
    ...
    setLoading(true)
    const res = await axios.post('https://ssukzip.deta.dev/review/create', data, {
      headers: {
        "Authorization": `Bearer ${access_token}`
      }
    })
...
function LikeButton({id}: {id: number}) {
  ...
  const handleLike = async () => {
    const access_token = getCookie('access_token')
    const data = await axios.get(`https://ssukzip.deta.dev/review/recommend/${id}`, {
      headers: {
        "Authorization": `Bearer ${access_token}`
      }
    }).then(res => res.data)
